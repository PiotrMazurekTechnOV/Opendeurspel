...
def vrgbutton():
    ...
    vraagzin1 = select_database("vraag", "questions", 20)
...
def klbutton():
    ...
    for i in antwoord1.curselection():
        ...
        my_conn.execute(("SELECT juist_ant FROM answer WHERE question_id = 20 AND answertext = %s"),(an1,))
    ...
    #my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 2 ,result + %s)")
                    #,(IDin, punt))
    my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 20,result + %s)")
                    ,(IDin, punt1))
    my_connect.commit()
...
my_conn.execute(("SELECT answertext FROM answer WHERE question_id = 20"))
