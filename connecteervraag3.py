...
def vrgbutton():
    ...
    vraagzin1 = select_database("vraag", "questions", 3)
...
def klbutton():
    ...
    for i in antwoord1.curselection():
        ...
        my_conn.execute(("SELECT juist_ant FROM answer WHERE question_id = 3 AND answertext = %s"),(an1,))
    ...
    #my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 2 ,result + %s)")
                    #,(IDin, punt))
    my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 3 ,result + %s)")
                    ,(IDin, punt1))
    my_connect.commit()
...
klaarbutton = Button(root, text='klaar', command=klbutton
                     ,bg="#502E93", fg="#F9DEE3", font=("Constantia",25),relief = GROOVE)
vraagbutton = Button(root, text='Ga naar de vraag'
                     ,bg="#502E93", fg="#F9DEE3", font=("Constantia",20),relief = GROOVE
                     , command=vrgbutton)
...
my_conn.execute(("SELECT answertext FROM answer WHERE question_id = 3"))
