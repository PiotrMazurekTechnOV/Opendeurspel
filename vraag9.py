...
def vrgbutton():
    ...
    vraagzin = select_database("vraag", "questions", 9)

    #checkt wat het antwoord is
    my_conn.execute(("SELECT answertext FROM answer WHERE question_id = 9 "))
...
def klbutton():
    ...
    my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 9 ,result + %s)")
                    ,(IDin, punt))
    my_connect.commit()
...
vraagbutton = Button(root, text='Ga naar de vraag',bg="#502E93", fg="#F9DEE3", font=("Constantia",20),relief = GROOVE
                     , command=vrgbutton)
