...
def vrgbutton():
    ...
    vraagzin = select_database("vraag", "questions", 7)

    #checkt wat het antwoord is
    my_conn.execute(("SELECT answertext FROM answer WHERE question_id = 7 "))
...
def klbutton():
    ...
    my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 7 ,result + %s)")
                    ,(IDin, punt))
    my_connect.commit()
