...
def vrgbutton():
    ...
    vraagzin1 = select_database("vraag", "questions", 16)
...
def klbutton():
    ...
    for i in antwoord1.curselection():
        ...
        my_conn.execute(("SELECT juist_ant FROM answer WHERE question_id = 16 AND answertext = %s"),(an1,))
    ...
    #my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 2 ,result + %s)")
                    #,(IDin, punt))
    my_conn.execute(("INSERT INTO results (user_id, question_id, result) VALUES(%s, 16 ,result + %s)")
                    ,(IDin, punt1))
    my_connect.commit()
...
vraag1 = Label(root, text = "wat is 7-1"
               ,bg="#8C4509", fg="#FBF4D7", font=("Constantia",20),relief = GROOVE)

#alle antwoorden
#eerst fetch de getallen
my_conn.execute(("SELECT answertext FROM answer WHERE question_id = 16"))
